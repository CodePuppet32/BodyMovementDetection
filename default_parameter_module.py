...
def set_default_vars(self):
    ...
    def_parameter_list = ['Jump N Frames', 'Detection Speed', 'Backtrack N Frames', 'Read nth Frame',
                          ...
                          'Show After', 'Detection After N', 'Progress Bar Speed']

    # color_list = ['#26E3F7', '#8544F7', '#F75757', '#F71FB1', '#0CF710', '#26E3F7', '#8544F7', '#F75757', '#F71FB1']

    parameter_value_vars = [IntVar() for i in range(9)]
    ...
    parameter_value_vars[9].set(self.progress_bar_speed)

    for i, parameter in enumerate(def_parameter_list):
        Label(content_frame, text=parameter.upper(), font=default_text_font_bold, foreground='white', background=background).grid(row=i, column=0, ipady=10, ipadx=10, sticky='ew')
        Entry(content_frame, textvariable=parameter_value_vars[i], font=button_font, width=5, borderwidth=1, background='white', relief='raised', foreground='#224275').grid(row=i, column=1)
    ...
    Button(button_frame, default_button, text='Save', command=lambda: apply(self, parameter_value_vars)).pack(side=LEFT, padx=(0, 5))
    Button(button_frame, default_button, text='Close', command=lambda: (stop_progress_bar(), set_default_vars_window.destroy())).pack(side=LEFT, padx=(5, 0))
    button_frame.grid(row=10, column=0, columnspan=2, ipady=10)
...
def apply(self, parameter_value_vars):
    ...
    self.progress_bar_speed = float(parameter_value_vars[9].get())
