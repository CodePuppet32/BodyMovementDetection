...
from imageai.Detection.YOLO.utils import letterbox_image, yolo_eval, preprocess_input, retrieve_yolo_detections, \
    draw_boxes
...
class ObjectDetection:
    ...
    * loadModel() [This must be called once only before performing object detection]
        ...
        if (self.__modelLoaded == False):
            ...
            try:
                ...
                for detection in detections:
                    detection_set.add(detection['name'])

                if output_type == "file":
                    for detection in detection_set:
                        output_image_path += ',' + detection + ','
                    cv2.imwrite(output_image_path + '.jpg', image_copy)
    ...
    def detectCustomObjectsFromImage(self, input_image="", output_image_path="", input_type="file", output_type="file",
        ...
        warnings.warn(
            "'detectCustomObjectsFromImage()' function has been deprecated and will be removed in future versions of ImageAI. \n Kindly use 'detectObjectsFromImage()' ",
...
class VideoObjectDetection:
    ...
    def detectCustomObjectsFromVideo(self, input_file_path="", camera_input=None, output_file_path="",
